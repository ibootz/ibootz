...
public class PingController extends BaseController {
	...
	@Autowired
	private PingElasticService pingOrderService;
	...
	private boolean testElastic() {
		...
		// 模拟查询
		List<PingElastic> pingOrders = null;
		try {
			pingOrders = pingOrderService.findByCreateTimeBetweenOrderByCreateTimeDesc(startTime, LocalDateTime.now());
		...
		try {
			// 删除数据
			pingOrderService.deleteOrders(pingOrders);
		} catch (Exception e) {
	...
	private void randomData(int mockDataCount) {
		for (int i = 0; i < mockDataCount; i++) {
			...
			ping.setCreateTime(RandomHelper.randomDateTime(LocalDateTime.of(2018, 01, 01, 0, 0), LocalDateTime.now()));
